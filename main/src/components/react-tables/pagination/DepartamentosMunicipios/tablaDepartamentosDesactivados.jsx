...
const MunicipiosPaginationTable = () => {
    ...
    useEffect(() => {
        const fetchMunicipios = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${URL}departamentos/desactivados`);
    ...
    const handleActivate = async (id) => {
        setLoading(true);
        try {
            await axios.put(`${URL}departamentos/activar/${id}`);
            setData((prevData) => prevData.map((item) => 
                item.id === id ? { ...item, estado: true } : item
            ));
        } catch (error) {
            console.error("Error al activar municipio:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        setFilteredData(
            data.filter((departamento) =>
                !departamento.estado && departamento.nombre.toLowerCase().includes(searchQuery.toLowerCase())
            )
        );
    }, [searchQuery, data]);
