...
const LineasDesactivadoPaginationTable = () => {
    ...
    useEffect(() => {
        const fetchLineas = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${URL}lineasEstrategicas/desactivados`);
    ...
    const handleActivate = async (id) => {
        setLoading(true);
        try {
            await axios.put(`${URL}lineasEstrategicas/activar/${id}`);
    ...
    useEffect(() => {
        setFilteredData(
            data.filter((lineasEstrategicas) =>
                !lineasEstrategicas.estado && lineasEstrategicas.nombre.toLowerCase().includes(searchQuery.toLowerCase())
            )
        );
    }, [searchQuery, data]);
    ...
    return (
        <Box>
            <Grid container spacing={3}>
                <Grid item xs={12}>
                    <TextField
                        label="Buscar Linea estrategica"
                        variant="outlined"
                        fullWidth
                        value={searchQuery}
...
export default LineasDesactivadoPaginationTable;
