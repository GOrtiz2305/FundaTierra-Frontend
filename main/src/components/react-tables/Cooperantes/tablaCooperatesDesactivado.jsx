...
const RubrosDesactivadoPaginationTable = () => {
    ...
    useEffect(() => {
        const fetchLineas = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${URL}cooperante/desactivados`);
    ...
    const handleActivate = async (id) => {
        setLoading(true);
        try {
            await axios.put(`${URL}cooperante/activar/${id}`);
    ...
    useEffect(() => {
        setFilteredData(
            data.filter((cooperante) =>
                !cooperante.estado && cooperante.nombre_donante.toLowerCase().includes(searchQuery.toLowerCase())
            )
        );
    }, [searchQuery, data]);

    const columns = [
        columnHelper.accessor('nombre_donante', {
            header: 'Nombre',
            cell: (info) => (
