...
import CustomTextField from '../theme-elements/CustomTextField';
  ...
  const LineasEstrategicaEditarForm = ({ id }) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`${URL}proyectoLinea/${id}`);
          if (!response.ok) {
    ...
    const handleSave = async (values) => {
      try {
        ...
        if (response.ok) {
          ...
          alert('Línea estratégica actualizada con éxito');
        } else {
          alert('Error al actualizar la línea estratégica');
        }
      } catch (error) {
  ...
  export default LineasEstrategicaEditarForm;